...
# ---------------------------------------------------------
# 2. NORMALISATION DES DONNÉES & CRÉATION DES SÉQUENCES 3D
# ---------------------------------------------------------
scaler_X = StandardScaler()
scaler_y = StandardScaler()

X1_train_scaled = scaler_X.fit_transform(X1_train_raw)
X1_test_scaled = scaler_X.transform(X1_test_raw)

Y1_train_scaled = scaler_y.fit_transform(Y1_train_raw)

# Fonction de conversion en structure 3D : (Échantillons, Pas de temps, Caractéristiques)
...
TIME_STEPS = 1
X1_train_3d, Y1_train_3d = create_dataset(X1_train_scaled, Y1_train_scaled, TIME_STEPS)

X1_test_full = np.vstack([X1_train_scaled[-TIME_STEPS+1:], X1_test_scaled]) if TIME_STEPS > 1 else X1_test_scaled
X1_test_3d, _ = create_dataset(X1_test_full, Y1_test.reshape(-1, 1), TIME_STEPS)

# ---------------------------------------------------------
# 3. CONSTRUCTION ET ENTRAÎNEMENT DU MODÈLE GRU
# ---------------------------------------------------------
model_ecvalue = Sequential([
    GRU(64, activation='tanh', input_shape=(X1_train_3d.shape[1], X1_train_3d.shape[2]), return_sequences=False),
    Dropout(0.2),
...
# Entraînement du réseau GRU
history = model_ecvalue.fit(
    X1_train_3d, Y1_train_3d,
    epochs=20,
    batch_size=64,
...
pred_scaled = model_ecvalue.predict(X1_test_3d)
...
print("\n=========================================================")
print("--- RÉSULTATS : GRU REGRESSION (ec_value) ---")
print(f"[ec_value] RMSE: {r1:.4f} | MAE: {m1:.4f} | R²: {r2_1:.4f}")

# Extraction relative basée sur la magnitude moyenne des poids de la première couche GRU
gru_weights = model_ecvalue.layers[0].get_weights()[0]
feature_importances_vals = np.mean(np.abs(gru_weights), axis=1)
...
print("\n--- TOP 5 DES CARACTÉRISTIQUES LES PLUS INFLUENTES (ESTIMATION GRU) ---")
...
# ---------------------------------------------------------
subtitle_1 = f"<b>ec_value : Réel vs GRU Regression</b><br><span style='font-size: 11px; color: #555;'>RMSE: {r1:.2f} | MAE: {m1:.2f} | R²: {r2_1:.4f}</span>"
...
fig.update_layout(
    ...
    title_text="<b>PRÉVISION THERMIQUE (EC_VALUE) PAR GRU (TEST 2026)</b>",
    title_x=0.5,
    hovermode="x unified",
