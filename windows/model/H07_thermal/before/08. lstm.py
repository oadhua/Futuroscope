...
# Fonction de transformation en tenseur 3D : (Échantillons, Pas de temps, Caractéristiques)
...
model_ecvalue = Sequential([
    LSTM(64, activation='relu', input_shape=(X1_train_3d.shape[1], X1_train_3d.shape[2]), return_sequences=False),
    Dropout(0.2),
...
# Prédiction et dé-normalisation vers l'échelle d'origine
...
print("\n=========================================================")
print("--- RÉSULTATS : LSTM REGRESSION (ec_value) ---")
print(f"[ec_value] RMSE: {r1:.4f} | MAE: {m1:.4f} | R²: {r2_1:.4f}")

# Extraction relative basée sur la magnitude moyenne des poids de la première couche LSTM
lstm_weights = model_ecvalue.layers[0].get_weights()[0]
feature_importances_vals = np.mean(np.abs(lstm_weights), axis=1)
...
print("\n--- TOP 5 DES CARACTÉRISTIQUES LES PLUS INFLUENTES (ESTIMATION LSTM) ---")
...
# ---------------------------------------------------------
subtitle_1 = f"<b>ec_value : Réel vs LSTM Regression</b><br><span style='font-size: 11px; color: #555;'>RMSE: {r1:.2f} | MAE: {m1:.2f} | R²: {r2_1:.4f}</span>"
...
fig.update_layout(
    ...
    title_text="<b>PRÉVISION THERMIQUE (EC_VALUE) PAR LSTM (TEST 2026)</b>",
    title_x=0.5,
    hovermode="x unified",
