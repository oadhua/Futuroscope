...
# ---------------------------------------------------------
# 2. ENTRAÎNEMENT DU MODÈLE GRADIENT BOOSTING
# ---------------------------------------------------------
model_ecvalue = GradientBoostingRegressor(
    n_estimators=100, 
    learning_rate=0.1, 
    max_depth=3, 
    random_state=42
...
print("\n=========================================================")
print("--- RÉSULTATS : GRADIENT BOOSTING REGRESSION (ec_value) ---")
print(f"[ec_value] RMSE: {r1:.4f} | MAE: {m1:.4f} | R²: {r2_1:.4f}")

# Extraction du Top 5 Feature Importance de Gradient Boosting
...
# ---------------------------------------------------------
subtitle_1 = f"<b>ec_value : Réel vs Gradient Boosting Regression</b><br><span style='font-size: 11px; color: #555;'>RMSE: {r1:.2f} | MAE: {m1:.2f} | R²: {r2_1:.4f}</span>"
...
# Courbe Prédite (Gradient Boosting)
fig.add_trace(
    go.Scatter(
        x=test_dates,
        y=pred_ecvalue,
        mode="lines",
        name="ec_value Prédiction (Gradient Boosting)",
...
fig.update_layout(
    ...
    title_text="<b>PRÉVISION THERMIQUE (EC_VALUE) PAR GRADIENT BOOSTING (TEST 2026)</b>",
    title_x=0.5,
    hovermode="x unified",
