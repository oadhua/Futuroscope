...
# ---------------------------------------------------------
# 2. NORMALISATION DES DONNÉES ET ENTRAÎNEMENT DU KNN
# ---------------------------------------------------------
# KNN étant sensible aux échelles de distance, la standardisation est indispensable
scaler = StandardScaler()
X1_train = scaler.fit_transform(X1_train_raw)
X1_test = scaler.transform(X1_test_raw)

# Initialisation du modèle KNN Regressor
model_ecvalue = KNeighborsRegressor(n_neighbors=5, weights='uniform', n_jobs=-1)
model_ecvalue.fit(X1_train, Y1_train)

pred_ecvalue = model_ecvalue.predict(X1_test)

# ---------------------------------------------------------
# 3. ÉVALUATION DES PERFORMANCES ET PERMUTATION IMPORTANCE
# ---------------------------------------------------------
...
print("\n=========================================================")
print("--- RÉSULTATS : K-NEAREST NEIGHBORS REGRESSION (KNN - ec_value) ---")
print(f"[ec_value] RMSE: {r1:.4f} | MAE: {m1:.4f} | R²: {r2_1:.4f}")

# Calcul du Top 5 des caractéristiques via Permutation Importance (spécifique aux modèles non-arborescents)
perm_importance = permutation_importance(
    model_ecvalue, X1_test, Y1_test, n_repeats=5, random_state=42, n_jobs=-1
)
...
print("\n--- TOP 5 DES CARACTÉRISTIQUES LES PLUS INFLUENTES (PERMUTATION IMPORTANCE) ---")
...
# ---------------------------------------------------------
subtitle_1 = f"<b>ec_value : Réel vs K-Nearest Neighbors (KNN)</b><br><span style='font-size: 11px; color: #555;'>RMSE: {r1:.2f} | MAE: {m1:.2f} | R²: {r2_1:.4f}</span>"
...
fig.update_layout(
    ...
    title_text="<b>PRÉVISION THERMIQUE (EC_VALUE) PAR KNN (TEST 2026)</b>",
    title_x=0.5,
    hovermode="x unified",
