...
# Đọc file
df = pd.read_csv("D:\Stage SI\Machine Learning\Futuroscope\windows\donne_clean\master_ml\master_H03_predict_pure_elec.csv")
...
y_train_1 = df.loc[train_mask, "elec_1"]
X_test = df.loc[test_mask, raw_features]
y_test_1 = df.loc[test_mask, "elec_1"]

# Target 2: elec_2
y_train_2 = df.loc[train_mask, "elec_2"]
y_test_2 = df.loc[test_mask, "elec_2"]
...
# Train XGBoost cho elec_2
xgb_2 = XGBRegressor(n_estimators=300, learning_rate=0.04, max_depth=6, random_state=42)
...
r2, m2, r2_2 = get_metrics(y_test_2, pred_2)
