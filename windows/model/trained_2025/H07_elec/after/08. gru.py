...
print(f"Dimensions des données d'entrée GRU pour H07 :")
print(f" -> X_train: {X_train_global.shape} (Echantillons, Pas de temps, Variables)")
print(f" -> X_test : {X_test_global.shape}\n")

# ==========================================================
# 5. Construction et entraînement du modèle GRU (3 Outputs)
# ==========================================================
model = Sequential()
...
print("--- DEBUT DE L'ENTRAINEMENT DU GRU GLOBAL H07 ---")
history = model.fit(
    X_train_global, 
    y_train_global,
    epochs=50,
...
print("\n=== RÉSULTATS DU MODÈLE GLOBAL GRU H07 - MULTI-OUTPUT ===")
...
titre_graphe = f"Prévisions Globales (elec_1 & elec_2 & elec_3) via GRU | elec_1_R²: {r2_chi_tiet[0]:.4f} - MAE: {mae_chi_tiet[0]:.2f} | elec_2_R²: {r2_chi_tiet[1]:.4f} - MAE: {mae_chi_tiet[1]:.2f} | elec_3_R²: {r2_chi_tiet[2]:.4f} - MAE: {mae_chi_tiet[2]:.2f}"
fig.update_layout(
    title=titre_graphe,
    xaxis_title="Date",
...
fig.write_html("prévisions_gru_elec_H07.html")
