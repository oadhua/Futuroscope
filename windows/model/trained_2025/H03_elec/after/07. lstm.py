...
print("--- DEBUT DE L'ENTRAINEMENT DU LSTM GLOBAL ---")
history = model.fit(
    X_train_global, 
    y_train_global,
    epochs=50,
...
print("\n=== RÉSULTATS DU MODÈLE GLOBAL LSTM - MULTI-OUTPUT ===")
...
titre_graphe = f"Prévisions Globales (elec_1 & elec_2) via LSTM | elec_1_R²: {r2_chi_tiet[0]:.4f} - MAE: {mae_chi_tiet[0]:.2f} | elec_2_R²: {r2_chi_tiet[1]:.4f} - MAE: {mae_chi_tiet[1]:.2f}"
fig.update_layout(
    title=titre_graphe,
    xaxis_title="Date",
...
fig.write_html("prévisions_lstm_elec.html")
