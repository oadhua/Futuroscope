...
MODEL_DIR = os.path.join(DATA_OUT, "01. linear_regression")

os.makedirs(MODEL_DIR, exist_ok=True)

print("=== HUẤN LUYỆN LINEAR REGRESSION & BACKCASTING QUÁ KHỨ (SỐ NGUYÊN) ===")
...
COLOR_PALETTE = {
    'H03': {'actual': '#1f77b4', 'pred_lr': '#ff7f0e'}, 
    'H07': {'actual': '#d62728', 'pred_lr': '#2ca02c'} 
}
...
for idx, att_id in enumerate(df['id_attraction'].unique()):
    ...
    # Khởi tạo cột chứa giá trị dự báo backcast bằng Linear Regression nếu chưa có
    sub_df['visitor_count_backcast_lr'] = np.nan
    ...
    # Huấn luyện mô hình đánh giá (Validation) với Linear Regression
    model_lr_eval = LinearRegression()
    model_lr_eval.fit(X_train, y_train)
    
    # LÀM TRÒN THÀNH SỐ NGUYÊN (VALIDATION)
    y_pred_lr = np.round(np.clip(model_lr_eval.predict(X_test), a_min=0, a_max=None)).astype(int)
    
    # Lưu chỉ số đánh giá Validation
    mae = mean_absolute_error(y_test, y_pred_lr)
    rmse = np.sqrt(mean_squared_error(y_test, y_pred_lr))
    r2 = r2_score(y_test, y_pred_lr)
    results_summary.append({'Attraction': att_id, 'Model': 'Linear Regression', 'MAE': mae, 'RMSE': rmse, 'R2': r2})
    print(f" -> [Linear Regression] Validation R²: {r2:.4f} | MAE: {mae:.2f}")
    ...
    final_model_lr.fit(X_all_recent, y_all_recent)
    ...
    y_past_pred_lr = np.array([])
    
    if backcast_mask.any():
        X_backcast = sub_df.loc[backcast_mask, FEATURE_COLS]
        
        # LÀM TRÒN THÀNH SỐ NGUYÊN (BACKCASTING)
        pred_lr_backcast = np.round(np.clip(final_model_lr.predict(X_backcast), 0, None)).astype(int)
        
        sub_df.loc[backcast_mask, 'visitor_count_backcast_lr'] = pred_lr_backcast
        
        # Lấp đầy trực tiếp vào cột TARGET nếu dòng đó đang trống / quá khứ
        sub_df.loc[backcast_mask, TARGET] = pred_lr_backcast
        
        past_df = sub_df[backcast_mask].copy()
        y_past_pred_lr = pred_lr_backcast
        print(f" -> ⏪ Đã Backcast bằng Linear Regression (số nguyên) cho {backcast_mask.sum():,} dòng (quá khứ hoặc thiếu).")

    filled_dfs.append(sub_df)

    # --- VẼ ĐỒ THỊ PLOTLY ---
    colors = COLOR_PALETTE.get(att_id, {'actual': '#1f77b4', 'pred_lr': '#ff7f0e'})
    is_visible = True if idx == 0 else 'legendonly'

    # 1. Trace Backcast quá khứ (Linear Regression)
    if len(past_df) > 0:
        fig.add_trace(go.Scatter(
            x=past_df['datetime'], y=y_past_pred_lr,
            mode='lines', name=f'[{att_id}] Backcast (LR)',
            line=dict(color=colors['pred_lr'], width=1.5, dash='dashdot'),
            visible=is_visible,
            hovertemplate=f'<b>{att_id} Backcast (LR):</b> %{{y}} người<br><b>Thời gian:</b> %{{x}}<extra></extra>'
        ))
    ...
    # 3. Trace Dự báo Validation (Linear Regression)
    fig.add_trace(go.Scatter(
        x=test_df['datetime'], y=y_pred_lr,
        mode='lines', name=f'[{att_id}] Dự báo (LR)',
        line=dict(color=colors['pred_lr'], width=1.5, dash='dot'),
        visible=is_visible,
        hovertemplate=f'<b>{att_id} LR:</b> %{{y}} người<br><b>Thời gian:</b> %{{x}}<extra></extra>'
    ))

# Gộp toàn bộ dữ liệu sau khi backcast xong
final_full_dataset = pd.concat(filled_dfs, ignore_index=True)

# Ép kiểu toàn bộ cột TARGET thành số nguyên trước khi xuất file
...
# ==========================================
fig.update_layout(
    title='<b>BACKCASTING QUÁ KHỨ & VALIDATION (LINEAR REGRESSION)</b>',
...
# Lưu file kết quả vào thư mục riêng của Linear Regression
output_combined_html = os.path.join(MODEL_DIR, "01_linear_regression_backcast_complete.html")
output_csv = os.path.join(MODEL_DIR, "visitor_fully_backcasted_lr.csv")

fig.write_html(output_combined_html)
final_full_dataset.to_csv(output_csv, index=False, encoding='utf-8-sig')

summary_df = pd.DataFrame(results_summary)
summary_df.to_csv(os.path.join(MODEL_DIR, "lr_baseline_results_summary.csv"), index=False, encoding='utf-8-sig')

print("\n==================================================")
print("🎉 HOÀN THÀNH HUẤN LUYỆN, VALIDATE VÀ BACKCASTING VỚI LINEAR REGRESSION (SỐ NGUYÊN)!")
