...
# ---------------------------------------------------------
# 3. ENTRAÎNEMENT ET DÉROULEMENT DU MODÈLE KERAS GRU
# ---------------------------------------------------------
def train_and_predict_keras_gru(features, target_name):
    # Standardisation
    ...
    # Architecture Keras Sequential avec couches GRU
    model = Sequential([
    ...
    # Entraînement
    print(f"\n--- Entraînement du modèle Keras GRU pour [{target_name}] ---")
    model.fit(
...
# ---------------------------------------------------------
pred_elec_1, top5_elec_1 = train_and_predict_keras_gru(features_elec_1, "elec_1")
pred_elec_2, top5_elec_2 = train_and_predict_keras_gru(features_elec_2, "elec_2")
...
print("\n=========================================================")
print("--- RÉSULTATS : MODÈLES KERAS GRU INDÉPENDANTS ---")
...
# ---------------------------------------------------------
subtitle_1 = f"<b>elec_1 : Réel vs Keras GRU</b><br><span style='font-size: 11px; color: #555;'>RMSE: {r1:.2f} kWh | MAE: {m1:.2f} kWh | R²: {r2_1:.4f}</span>"
subtitle_2 = f"<b>elec_2 : Réel vs Keras GRU</b><br><span style='font-size: 11px; color: #555;'>RMSE: {r2:.2f} kWh | MAE: {m2:.2f} kWh | R²: {r2_2:.4f}</span>"
...
fig.add_trace(
    go.Scatter(
        ...
        y=pred_elec_1,
        mode="lines",
        name="elec_1 Prédiction (Keras GRU)",
...
fig.add_trace(
    go.Scatter(
        ...
        y=pred_elec_2,
        mode="lines",
        name="elec_2 Prédiction (Keras GRU)",
...
fig.update_layout(
    ...
    title_text="<b>PRÉVISION PAR KERAS GRU INDÉPENDANTS (TEST 2026)</b>",
    title_x=0.5,
    hovermode="x unified",
