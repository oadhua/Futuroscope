...
# ==========================================================
# 4. Entraînement du modèle XGBoost (Multi-output)
# ==========================================================
# Configuration des paramètres optimisés contre l'overfitting
modele = XGBRegressor(
    n_estimators=750,
    learning_rate=0.03,
    max_depth=3,
...
# Entraînement global (XGBoost prend directement en charge le multi-output)
...
print("=== RÉSULTATS DU MODÈLE GLOBAL XGBOOST - MULTI-OUTPUT ===")
print(f"XGBoost Train R² (Moyenne): {r2_train:.4f}")
print(f"XGBoost Test R²  (Moyenne): {r2_global:.4f} | MAE (Moyenne): {mae_global:.2f}")
print("-" * 65)
...
titre_graphe = f"Prévisions Globales (elec_1 & elec_2 & elec_3) via XGBoost | elec_1_R²: {r2_chi_tiet[0]:.4f} - MAE: {mae_chi_tiet[0]:.2f} | elec_2_R²: {r2_chi_tiet[1]:.4f} - MAE: {mae_chi_tiet[1]:.2f} | elec_3_R²: {r2_chi_tiet[2]:.4f} - MAE: {mae_chi_tiet[2]:.2f}"
fig.update_layout(
    title=titre_graphe,
    xaxis_title="Date",
...
fig.write_html("prévisions_xgb_elec.html")
