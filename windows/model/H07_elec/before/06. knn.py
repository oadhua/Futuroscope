...
# ---------------------------------------------------------
# 2. STANDARDISATION (MANDATOIRE POUR KNN)
# ---------------------------------------------------------
...
# ---------------------------------------------------------
# 3. ENTRAÎNEMENT DES MODÈLES KNN INDÉPENDANTS
# ---------------------------------------------------------
model_elec_1 = KNeighborsRegressor(n_neighbors=7, weights="distance", n_jobs=-1)
model_elec_1.fit(X1_train, Y1_train)
pred_elec_1 = model_elec_1.predict(X1_test)

model_elec_2 = KNeighborsRegressor(n_neighbors=7, weights="distance", n_jobs=-1)
model_elec_2.fit(X2_train, Y2_train)
pred_elec_2 = model_elec_2.predict(X2_test)

# ---------------------------------------------------------
# 4. ÉVALUATION DES PERFORMANCES ET PERMUTATION IMPORTANCE
# ---------------------------------------------------------
...
print("\n--- RÉSULTATS : MODÈLES KNN INDÉPENDANTS ---")
print(f"[elec_1] RMSE: {r1:.4f} kWh | MAE: {m1:.4f} kWh | R²: {r2_1:.4f}")
print(f"[elec_2] RMSE: {r2:.4f} kWh | MAE: {m2:.4f} kWh | R²: {r2_2:.4f}")

# Calcul de l'importance par Permutation pour elec_1 (sur le jeu de test)
perm_importance_1 = permutation_importance(
    model_elec_1, X1_test, Y1_test, n_repeats=5, random_state=42, n_jobs=-1
)
importances_1 = pd.DataFrame(
    {"Feature": features_elec_1, "Importance": perm_importance_1.importances_mean}
).sort_values(by="Importance", ascending=False)

print("\nTop 5 des caractéristiques les plus influentes pour [elec_1] (Permutation Importance) :")
print(importances_1.head(5).to_string(index=False))

# Calcul de l'importance par Permutation pour elec_2 (sur le jeu de test)
perm_importance_2 = permutation_importance(
    model_elec_2, X2_test, Y2_test, n_repeats=5, random_state=42, n_jobs=-1
)
importances_2 = pd.DataFrame(
    {"Feature": features_elec_2, "Importance": perm_importance_2.importances_mean}
).sort_values(by="Importance", ascending=False)

print("\nTop 5 des caractéristiques les plus influentes pour [elec_2] (Permutation Importance) :")
print(importances_2.head(5).to_string(index=False))

# ---------------------------------------------------------
# 5. VISUALISATION INTERACTIVE AVEC PLOTLY & EXPORT HTML
# ---------------------------------------------------------
subtitle_1 = f"<b>elec_1 : Réel vs KNN (K-Nearest Neighbors)</b><br><span style='font-size: 11px; color: #555;'>RMSE: {r1:.2f} kWh | MAE: {m1:.2f} kWh | R²: {r2_1:.4f}</span>"
subtitle_2 = f"<b>elec_2 : Réel vs KNN (K-Nearest Neighbors)</b><br><span style='font-size: 11px; color: #555;'>RMSE: {r2:.2f} kWh | MAE: {m2:.2f} kWh | R²: {r2_2:.4f}</span>"
...
fig.update_layout(
    ...
    title_text="<b>PRÉVISION PAR KNN INDÉPENDANTS (TEST 2026)</b>",
    title_x=0.5,
    hovermode="x unified",
