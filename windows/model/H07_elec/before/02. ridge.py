import os
import numpy as np
import pandas as pd
from sklearn.linear_model import Ridge
from sklearn.metrics import mean_squared_error, mean_absolute_error, r2_score

import plotly.graph_objects as go
from plotly.subplots import make_subplots

# ---------------------------------------------------------
# 1. NẠP DỮ LIỆU & KHỞI TẠO DANH SÁCH FEATURES RIÊNG BIỆT
# ---------------------------------------------------------
file_path = r"D:\Stage SI\Machine Learning\Futuroscope\windows\donne_clean\master_ml\master_H07_predict_pure_elec.csv"
df = pd.read_csv(file_path)

if "date" in df.columns:
    df["date"] = pd.to_datetime(df["date"])

# =========================================================
# 🎯 CẤU HÌNH FEATURES ĐẦU VÀO RIÊNG CHO TỪNG TARGET
# =========================================================
features_elec_1 = [
    "year", "month", "day", "hour", "week", "is_weekend", "is_open", "jf",
    "frequentation_park_daily", "surface", "duree", "ouvert", "interrompu",
    "operation", "visitor_count", "temperature", "humidite", "rayonnement_solaire",
    "day_degree_cold", "day_degree_hot", "temp_max", "temp_min", "temp_moy",
    "humidite_max", "humidite_min", "humidite_moy", "freq_HF", "freq_MF", "freq_THF"
]

features_elec_2 = [
    "year", "month", "day", "hour", "week", "is_weekend", "is_open", "jf",
    "frequentation_park_daily", "surface", "duree", "ouvert", "interrompu",
    "operation", "visitor_count", "temperature", "humidite", "rayonnement_solaire",
    "day_degree_cold", "day_degree_hot", "temp_max", "temp_min", "temp_moy",
    "humidite_max", "humidite_min", "humidite_moy", "freq_HF", "freq_MF", "freq_THF"
]

# Chia tập Train (< 2026) và Test (>= 2026)
train_mask = df["year"] < 2026
test_mask = df["year"] >= 2026

# Tách X, Y độc lập cho elec_1
X1_train = df.loc[train_mask, features_elec_1]
X1_test = df.loc[test_mask, features_elec_1]
Y1_train = df.loc[train_mask, "elec_1"]
Y1_test = df.loc[test_mask, "elec_1"]

# Tách X, Y độc lập cho elec_2
X2_train = df.loc[train_mask, features_elec_2]
X2_test = df.loc[test_mask, features_elec_2]
Y2_train = df.loc[train_mask, "elec_2"]
Y2_test = df.loc[test_mask, "elec_2"]

test_dates = (
    df.loc[test_mask, "date"] if "date" in df.columns else df.loc[test_mask].index
)

# ---------------------------------------------------------
# 2. HUẤN LUYỆN 2 MÔ HÌNH RIDGE REGRESSION ĐỘC LẬP
# ---------------------------------------------------------
# Mô hình 1: Cho elec_1
model_elec_1 = Ridge(alpha=1.0, random_state=42)
model_elec_1.fit(X1_train, Y1_train)
pred_elec_1 = model_elec_1.predict(X1_test)

# Mô hình 2: Cho elec_2
model_elec_2 = Ridge(alpha=1.0, random_state=42)
model_elec_2.fit(X2_train, Y2_train)
pred_elec_2 = model_elec_2.predict(X2_test)

# ---------------------------------------------------------
# 3. ĐÁNH GIÁ HIỆU NĂNG & IN HỆ SỐ RIDGE COEFFS
# ---------------------------------------------------------
def get_metrics(y_true, y_pred):
    rmse = np.sqrt(mean_squared_error(y_true, y_pred))
    mae = mean_absolute_error(y_true, y_pred)
    r2 = r2_score(y_true, y_pred)
    return rmse, mae, r2

r1, m1, r2_1 = get_metrics(Y1_test, pred_elec_1)
r2, m2, r2_2 = get_metrics(Y2_test, pred_elec_2)

print("\n--- KẾT QUẢ: INDEPENDENT RIDGE REGRESSION MODELS ---")
print(f"[elec_1] RMSE: {r1:.4f} kWh | MAE: {m1:.4f} kWh | R²: {r2_1:.4f}")
print(f"[elec_2] RMSE: {r2:.4f} kWh | MAE: {m2:.4f} kWh | R²: {r2_2:.4f}")

# Hiển thị Top features quan trọng cho elec_1
coef_df1 = pd.DataFrame(
    {"Feature": features_elec_1, "Coefficient": model_elec_1.coef_}
).sort_values(by="Coefficient", key=abs, ascending=False)
print("\nTop 5 đặc trưng ảnh hưởng lớn nhất tới [elec_1]:")
print(coef_df1.head(5).to_string(index=False))

# Hiển thị Top features quan trọng cho elec_2
coef_df2 = pd.DataFrame(
    {"Feature": features_elec_2, "Coefficient": model_elec_2.coef_}
).sort_values(by="Coefficient", key=abs, ascending=False)
print("\nTop 5 đặc trưng ảnh hưởng lớn nhất tới [elec_2]:")
print(coef_df2.head(5).to_string(index=False))

# ---------------------------------------------------------
# 4. VẼ ĐỒ THỊ TƯƠNG TÁC & XUẤT FILE HTML
# ---------------------------------------------------------
fig = make_subplots(
    rows=2,
    cols=1,
    shared_xaxes=True,
    vertical_spacing=0.08,
    subplot_titles=(
        "<b>elec_1: Thực tế vs Ridge Regression</b>",
        "<b>elec_2: Thực tế vs Ridge Regression</b>",
    ),
)

# elec_1
fig.add_trace(
    go.Scatter(
        x=test_dates,
        y=Y1_test,
        mode="lines",
        name="elec_1 Thực tế",
        line=dict(color="#1f77b4", width=1.5),
        hovertemplate="<b>Thời gian:</b> %{x}<br><b>Thực tế:</b> %{y:.2f} kWh<extra></extra>",
    ),
    row=1,
    col=1,
)
fig.add_trace(
    go.Scatter(
        x=test_dates,
        y=pred_elec_1,
        mode="lines",
        name="elec_1 Dự báo (Ridge)",
        line=dict(color="#ff7f0e", width=1.5, dash="dot"),
        customdata=np.stack((Y1_test, np.abs(Y1_test - pred_elec_1)), axis=-1),
        hovertemplate="<b>Thời gian:</b> %{x}<br><b>Dự báo:</b> %{y:.2f} kWh<br><b>Lệch:</b> %{customdata[1]:.2f} kWh<extra></extra>",
    ),
    row=1,
    col=1,
)

# elec_2
fig.add_trace(
    go.Scatter(
        x=test_dates,
        y=Y2_test,
        mode="lines",
        name="elec_2 Thực tế",
        line=dict(color="#2ca02c", width=1.5),
        hovertemplate="<b>Thời gian:</b> %{x}<br><b>Thực tế:</b> %{y:.2f} kWh<extra></extra>",
    ),
    row=2,
    col=1,
)
fig.add_trace(
    go.Scatter(
        x=test_dates,
        y=pred_elec_2,
        mode="lines",
        name="elec_2 Dự báo (Ridge)",
        line=dict(color="#d62728", width=1.5, dash="dot"),
        customdata=np.stack((Y2_test, np.abs(Y2_test - pred_elec_2)), axis=-1),
        hovertemplate="<b>Thời gian:</b> %{x}<br><b>Dự báo:</b> %{y:.2f} kWh<br><b>Lệch:</b> %{customdata[1]:.2f} kWh<extra></extra>",
    ),
    row=2,
    col=1,
)

fig.update_layout(
    height=750,
    title_text="<b>DỰ BÁO BẰNG INDEPENDENT RIDGE REGRESSION (TẬP TEST 2026)</b>",
    title_x=0.5,
    hovermode="x unified",
    template="plotly_white",
    legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
)

fig.update_yaxes(title_text="kWh", row=1, col=1)
fig.update_yaxes(title_text="kWh", row=2, col=1)
fig.update_xaxes(rangeslider_visible=True, row=2, col=1)

# --- XUẤT RA FILE HTML ---
output_html = "02. ridge.html"
fig.write_html(output_html, include_plotlyjs="cdn")
print(f"\n[OK] Đã xuất đồ thị tương tác ra file: {os.path.abspath(output_html)}")

fig.show()