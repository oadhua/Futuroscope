...
# Exclusion des variables de fuite (leak) et temporelles brutes (on garde "hour")
variables_exclues = [
    "date",
    "elec_1",
    "elec_2",
...
# ==========================================================
# 4. Entraînement du modèle GBR (Multi-output)
# ==========================================================
...
print("=== RÉSULTATS DU MODÈLE GLOBAL GBR - MULTI-OUTPUT ===")
print(f"GBR Train R² (Moyenne): {r2_train:.4f}")
print(f"GBR Test R²  (Moyenne): {r2_global:.4f} | MAE (Moyenne): {mae_global:.2f}")
print("-" * 65)
...
titre_graphe = f"Prévisions Globales (elec_1 & elec_2 & elec_3) via GBR | elec_1_R²: {r2_chi_tiet[0]:.4f} - MAE: {mae_chi_tiet[0]:.2f} | elec_2_R²: {r2_chi_tiet[1]:.4f} - MAE: {mae_chi_tiet[1]:.2f} | elec_3_R²: {r2_chi_tiet[2]:.4f} - MAE: {mae_chi_tiet[2]:.2f}"
fig.update_layout(
    title=titre_graphe,
    xaxis_title="Date",
...
fig.write_html("prévisions_gbr_elec.html")
