...
# ==========================================================
# Initialisation du modèle Random Forest avec des hyperparamètres robustes contre l'overfitting
...
modele = MultiOutputRegressor(base_rf)
...
print("=== RÉSULTATS DU MODÈLE GLOBAL RANDOM FOREST - MULTI-OUTPUT ===")
print(f"RF Train R² (Moyenne): {r2_train:.4f}")
print(f"RF Test R²  (Moyenne): {r2_global:.4f} | MAE (Moyenne): {mae_global:.2f}")
print("-" * 65)
...
titre_graphe = f"Prévisions Globales (elec_1 & elec_2 & elec_3) via Random Forest | elec_1_R²: {r2_chi_tiet[0]:.4f} - MAE: {mae_chi_tiet[0]:.2f} | elec_2_R²: {r2_chi_tiet[1]:.4f} - MAE: {mae_chi_tiet[1]:.2f} | elec_3_R²: {r2_chi_tiet[2]:.4f} - MAE: {mae_chi_tiet[2]:.2f}"
fig.update_layout(
    title=titre_graphe,
    xaxis_title="Date",
...
fig.write_html("prévisions_rf_elec.html")
