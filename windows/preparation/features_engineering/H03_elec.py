...
def preparer_features_communes(df):
    ...
    if 'type_frequentation' in df.columns:
        df = pd.get_dummies(df, columns=['type_frequentation'], prefix='freq', drop_first=True, dtype=int)
...
if __name__ == "__main__":
    df_raw = pd.read_csv(r"D:\Stage SI\Machine Learning\Futuroscope\windows\donne_clean\master_outliers\H03_elec_outliers.csv")
