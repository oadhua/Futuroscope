...
# ==============================================================================
# 1. CẤU HÌNH ĐƯỜNG DẪN & ĐỌC DỮ LIỆU THERMAL
# ==============================================================================
TARGET_ATTRACTION = "H03"
BASE_DIR = r"D:\Stage SI\Machine Learning\Futuroscope\windows\donne_clean"

master_path = os.path.join(BASE_DIR, "master", f"master_{TARGET_ATTRACTION}_thermal.csv")

if not os.path.exists(master_path):
    raise FileNotFoundError(f"Không tìm thấy file Master Nhiệt: {master_path}")

df_master = pd.read_csv(master_path)

print("=" * 70)
print(f"1. BÁO CÁO DỮ LIỆU CÒN THIẾU BAN ĐẦU - {TARGET_ATTRACTION} THERMAL (Tổng số dòng: {len(df_master)})")
print("=" * 70)
...
# ==============================================================================
# 2. XỬ LÝ NỘI SUY DỮ LIỆU NĂNG LƯỢNG NHIỆT (EC_VALUE) & RÀNG BUỘC LOGIC
# ==============================================================================
print("2. TIẾN HÀNH XỬ LÝ DỮ LIỆU NĂNG LƯỢNG NHIỆT & RÀNG BUỘC LOGIC...")

# 2.1. Nội suy tuyến tính cho biến ec_value (nếu có gap nhỏ)
if "ec_value" in df_master.columns:
    df_master["ec_value"] = df_master["ec_value"].interpolate(method="linear", limit_direction="both").fillna(0.0)
    # Đảm bảo tiêu thụ nhiệt không bị âm do nội suy
    df_master["ec_value"] = df_master["ec_value"].clip(lower=0.0)
...
# ==============================================================================
print("\n" + "=" * 70)
print(f"3. BÁO CÁO DỮ LIỆU SAU KHI XỬ LÝ MISSING VALUE - THERMAL")
print("=" * 70)
...
FEATURE_COLUMNS_ORDER = [
    ...
    # 6. Biến Target Năng lượng Nhiệt
    "ec_value"
]
...
output_clean_path = os.path.join(output_dir, f"{TARGET_ATTRACTION}_thermal_missing.csv")

df_master_clean.to_csv(output_clean_path, index=False, encoding="utf-8-sig")
print(f"-> ✅ Đã xuất file Master Nhiệt sạch hoàn chỉnh: {output_clean_path}")
...
# # ==============================================================================
# master_path = r"D:\Stage SI\Machine Learning\Futuroscope\windows\donne_clean\master\master_H03_thermal.csv"
# horaire_path = r"D:\Stage SI\Machine Learning\Futuroscope\windows\donne_clean\cadence\horaire_all_years.csv"
...
# # Bỏ các cột h_ouv, h_ferm cũ nếu đã có trong df_master
...
# print("=" * 65)
# print(f"1. BÁO CÁO DỮ LIỆU CÒN THIẾU BAN ĐẦU - THERMAL (Tổng số dòng: {len(df_master)})")
# print("=" * 65)
# null_before = df_master.isnull().sum()
# percent_before = (null_before / len(df_master)) * 100
# df_before_report = pd.DataFrame({
...
# # ==============================================================================
# # 2. XỬ LÝ NỘI SUY DỮ LIỆU MÉTÉO - THERMAL - OPERATIONAL
# # ==============================================================================
# # --- VARIABLE 1 : MÉTÉO ---
# colonnes_meteo = [
...
# # --- VARIABLE 2 : NĂNG LƯỢNG NHIỆT (ec_value) ---
# if "ec_value" in df_master.columns:
...
# # ==============================================================================
# # 3. CHỈ DỰ BÁO CÁC VỊ TRÍ BỊ THÍỆU (NaN) TRONG VISITOR_COUNT BẰNG ML
# # ==============================================================================
# if "visitor_count" in df_master.columns:
#     print("-> Tiến hành lọc và chỉ dự báo các vị trí bị thiếu trong visitor_count (Thermal)...")

#     # 1. Đánh dấu CHÍNH XÁC những dòng ban đầu bị NaN
#     mask_missing = df_master["visitor_count"].isnull()

#     # 2. Xử lý các dòng BỊ THÍEU theo điều kiện logic
...
# # ==============================================================================
# # 4. EXPORT FILE KẾT QUẢ THERMAL
# # ==============================================================================
# ordre_colonnes = [
#     "date", "visitor_count", "frequentation_park_daily", "surface",
#     "ec_value", "interrompu", "ouvert", "operation",
...
# os.makedirs(output_dir, exist_ok=True)
# output_clean_path = os.path.join(output_dir, "master_H03_thermal_missing.csv")

# df_master_clean.to_csv(output_clean_path, index=False)
# print(f"-> Xuất file Nhiệt thành công: {output_clean_path}")