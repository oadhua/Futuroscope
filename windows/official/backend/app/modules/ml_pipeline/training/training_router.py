...
@router.get(
    "/attractions",
    summary="Lấy danh sách tất cả id_attraction khả dụng trong hệ thống",
)
def get_attractions_list():
    """
    Trả về danh sách danh mục các id_attraction thực tế có trong cơ sở dữ liệu.
    """
    try:
        attractions = MLTrainingService.get_distinct_attractions()
        # Trả về kèm tùy chọn ALL mặc định
        return {
            "status": "success",
            "attractions": [attr for attr in attractions]
        }
...
@router.get(
    "/versions",
    summary="Lấy danh sách tất cả phiên bản dữ liệu khả dụng trong schema data_prep",
)
def get_data_prep_versions(
    id_attraction: Optional[str] = Query("ALL", description="Lọc phiên bản theo Attraction ID")
):
    """API giúp Frontend load dropdown chọn version_id linh hoạt từ DB."""
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Không thể lấy danh sách phiên bản từ DB: {str(e)}",
        )


@router.get(
    "/versions/{version_id}/columns",
    summary="Lấy danh sách các cột khả dụng trong phiên bản dữ liệu",
)
def get_version_columns(
    ...
    try:
        ...
        logger.error(f"Lỗi truy vấn danh sách cột của {version_id}: {str(e)}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Không thể lấy danh sách cột: {str(e)}",
        )
...
@router.delete(
    ...
    summary="Xóa một phiên bản dữ liệu (bảng) trong schema data_prep",
)
def delete_prep_version(
    version_id: str = Query(..., description="Mã phiên bản cần xóa"),
    id_attraction: Optional[str] = Query("ALL")
):
    try:
        ...
        return {
            "status": "success",
            "message": f"Phiên bản '{version_id}' đã được xóa thành công.",
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Không thể xóa phiên bản dữ liệu: {str(e)}",
        )
...
@router.post(
    ...
    summary="Huấn luyện mô hình ML/DL",
)
def run_training(req: TrainModelRequest):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Échec de l'entraînement du modèle : {str(e)}",
        )


@router.get(
    "/models",
    summary="Obtenir le registre de tous les modèles entraînés",
)
def list_trained_models(
    target_type: Optional[str] = Query(None, description="Filtrer par type: 'visitor' ou 'energy'"),
):
    try:
        where_clause = ""
        params = {}
...
@router.delete(
    "/models/{model_id}",
    status_code=status.HTTP_200_OK,
    summary="Xóa một mô hình đã huấn luyện theo model_id",
)
def delete_trained_model(model_id: str):
    try:
        ...
        return {
            "status": "success",
            "message": f"Mô hình '{model_id}' đã được xóa thành công.",
            "model_id": model_id,
        }
    except ValueError as ve:
        raise HTTPException(
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Không thể xóa mô hình: {str(e)}",
        )


@router.get(
    "/versions/{version_id}/date-range",
    summary="Lấy khoảng thời gian (min_date, max_date) của phiên bản dữ liệu",
)
def get_version_date_range(
    version_id: str, 
    id_attraction: Optional[str] = Query("ALL")
):
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Không thể lấy khoảng thời gian: {str(e)}",
        )