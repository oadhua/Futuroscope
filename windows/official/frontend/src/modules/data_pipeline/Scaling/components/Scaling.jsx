...
const STEP_TYPE = 'scaling';

const SCALING_METHODS = [
    { value: 'standard', label: 'Standardisation (Z-Score)' },
    { value: 'min_max', label: 'Mise à l\'échelle Min-Max' },
...
export default function ScalingModule() {
    ...
    // 3. Méthode de Scaling & Parameters
    ...
    // Lọc danh sách Version hiển thị ở CỘT BÊN PHẢI (Chỉ hiển thị các version thuộc Scaling)
    const moduleVersions = useMemo(() => {
        return Object.values(allVersions).filter(v => {
            ...
            const matchStep = step === STEP_TYPE ||
                ...
                method.includes('min_max') ||
                method.includes('robust');
    ...
    useEffect(() => {
        const fetchParentStats = async () => {
            ...
            try {
                const url = `${API_BASE}/scaling/versions/${selectedParent}/stats?id_attraction=${selectedAttraction}`;
                const res = await fetch(url);

                if (res.ok) {
                    const data = await res.json();

                    const cols = data.available_columns || Object.keys(data.null_counts || {});
    ...
    const handleExecute = async () => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(errorData.detail || 'Une erreur est survenue lors du traitement du scaling.');
            }
    ...
    const styles = {
        ...
        smallBtn: (active) => ({
            ...
            border: '1px solid', borderColor: active ? '#93c5fd' : '#e2e8f0',
        ...
        tabBtn: (isActive) => ({
            ...
            backgroundColor: isActive ? '#2563eb' : '#f1f5f9', color: isActive ? '#ffffff' : '#64748b', transition: 'all 0.15s ease', display: 'flex', alignItems: 'center', gap: '6px'
        }),
    ...
    const targetCols = useMemo(() => {
        ...
        return selectedVersion.target_columns || stats.columns_scaled || [];
    }, [selectedVersion]);
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
                            ...
                            <div>
                                <h1 style={{ margin: 0, fontSize: '18px', fontWeight: '800', color: '#0f172a', lineHeight: '1.2' }}>
                                    Standardisation et Mise à l'échelle (Scaling)
                                </h1>
                                <p style={{ margin: '4px 0 0 0', fontSize: '12px', color: '#94a3b8', fontWeight: '600', lineHeight: '1.2' }}>
                                    Standardisation (Z-score), Min-Max scaling et transformation robuste pour les caractéristiques numériques
                                </p>
                            </div>
                        </div>
                    </div>
                ...
                <div style={styles.gridContainer}>
                    ...
                    <div style={styles.card}>
                        <h2 style={{ margin: '0 0 16px 0', fontSize: '15px', fontWeight: '700', color: '#0f172a', borderBottom: '1px solid #f1f5f9', paddingBottom: '12px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                            <Sliders size={18} color="#2563eb" /> Configuration du Scaling
                        </h2>
                        ...
                        <div style={styles.formGroup}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                                <label style={{ ...styles.label, margin: 0 }}>
                                    3. Colonnes ({selectedColumns.length}/{availableColumns.length})
                                </label>
                        ...
                        <button onClick={handleExecute} disabled={loading} style={{ ...styles.actionBtn, backgroundColor: loading ? '#94a3b8' : '#2563eb' }}>
                            ...
                            {loading ? 'Traitement en cours...' : `Lancer le Scaling (${selectedColumns.length})`}
                        </button>
                            ...
                            <div style={styles.resultCard}>
                                ...
                                <div style={styles.kpiGrid}>
                                    <div style={styles.kpiCard}>
                                        <span style={styles.kpiTitle}>Lignes Modifiées</span>
                                    ...
                                    <div style={styles.kpiCard}>
                                        ...
                                        <span style={{ ...styles.kpiValue, fontSize: '13px', textTransform: 'uppercase', color: '#0284c7' }}>
                                ...
                                <div style={styles.tabHeader}>
                                    ...
                                    <button onClick={() => setActiveTab('columns')} style={styles.tabBtn(activeTab === 'columns')}>
                                        <BarChart2 size={14} /> Aperçu des échelles
                                    </button>
                                </div>
                                                ...
                                                {targetCols.length === 0 ? (
                                                    <tr>
                                                        <td colSpan="3" style={{ ...styles.td, textAlign: 'center', color: '#94a3b8' }}>
                                                            Toutes les colonnes numériques ont été mises à l'échelle.
                                                        </td>
                                                    </tr>
                                                ) : (
                                                    targetCols.map((col) => (
                                                        <tr key={col}>
                                                            ...
                                                            <td style={{ ...styles.td, textAlign: 'center', color: '#2563eb', fontWeight: '800', fontSize: '12px' }}>
                                                                {selectedVersion.method_label_fr || selectedVersion.method || 'Standard Scaling'}
                                                            </td>
                                                        </tr>
                                                    ))
                                                )}
                                            ...
                                            <div key={col} style={{ backgroundColor: '#ffffff', border: '1px solid #e2e8f0', padding: '12px 16px', borderRadius: '14px' }}>
                                                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '12px', fontWeight: '700', marginBottom: '6px' }}>
                                                    ...
                                                    <span style={{ color: '#2563eb' }}>
                                                        {selectedVersion.method === 'min_max'
                                                            ? `Plage cible : [${minRange}, ${maxRange}]`
                                                            : selectedVersion.method === 'robust' ? 'Médiane = 0, IQR = 1' : 'μ = 0, σ = 1'
                                                        }
                                                    </span>
                                                </div>
                                                <div style={{ height: '8px', backgroundColor: '#e2e8f0', borderRadius: '4px', overflow: 'hidden' }}>
                                                    <div style={{ width: '100%', height: '100%', backgroundColor: '#2563eb', borderRadius: '4px' }}></div>
                                                </div>
                                            </div>
                                        ))}
                                    </div>
